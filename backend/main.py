...
# Ledger-derived wallet trust list (field-tested), regenerated by
# scripts/build_wallet_trust.py. Whitelist = wallets our own win-ledger proves
# net-positive at a real sample; blacklist = proven money-losers. Loaded once
# and cached; the picks engine boosts the whitelist and excludes the blacklist.
_WALLET_TRUST_CACHE: dict = {}


def _load_wallet_trust() -> dict:
    if _WALLET_TRUST_CACHE.get("loaded"):
        return _WALLET_TRUST_CACHE
    path = os.path.join(os.path.dirname(__file__), "data", "wallet_trust.json")
    ...
    try:
        ...
        for e in data.get("whitelist", []):
            ...
            if addr:
                whitelist[addr] = e
        blacklist = {(e.get("address") or "").lower() for e in data.get("blacklist", []) if e.get("address")}
        meta = {"generated_at": data.get("generated_at"), "criteria": data.get("criteria")}
    except (FileNotFoundError, ValueError) as e:
        log_error("wallet_trust_load_failed", error=str(e)[:160])
    _WALLET_TRUST_CACHE.update({"loaded": True, "whitelist": whitelist, "blacklist": blacklist, "meta": meta})
    return _WALLET_TRUST_CACHE
...
def _ai_copy_score(move: dict, market: str) -> tuple[int, list[str]]:
    ...
    # Field-test boost: this exact wallet is on our ledger-derived whitelist —
    # PROVEN net-positive over a real sample of its own scored moves. This is the
    # strongest signal we have because it's our data, not an external score.
    trust = _load_wallet_trust()
    wl = trust.get("whitelist", {}).get((move.get("trader_address") or "").lower())
    if wl:
        score += 15
        reasons.append(
            f"proven winner in our ledger ({wl.get('win_rate_pct')}% over {wl.get('decisive')} scored moves)"
        )
...
async def invest_ai_picks(request: Request, limit: int = 6, conviction: str = "all"):
    ...
    # Field-test blacklist: drop wallets our own ledger has PROVEN to lose money
    # over a real sample (e.g. high-volume churners with negative net P&L). No
    # external score outranks our own outcomes.
    blacklist = _load_wallet_trust().get("blacklist", set())
    if blacklist:
        moves = [m for m in moves if (m.get("trader_address") or "").lower() not in blacklist]
    ...
    return success({
        ...
        "trust_foundation": {
            "proven_winners": len(trust.get("whitelist", {})),
            "excluded_losers": len(trust.get("blacklist", set())),
            "generated_at": trust.get("meta", {}).get("generated_at"),
        },
    })


@limiter.limit("60/minute")
@app.get("/api/invest/wallet-trust")
async def invest_wallet_trust(request: Request):
    """The field-tested foundation behind the picks feed: wallets our own
    win-ledger has PROVEN net-positive (whitelist, boosted) vs PROVEN losers
    (blacklist, excluded). Regenerated by scripts/build_wallet_trust.py."""
    path = os.path.join(os.path.dirname(__file__), "data", "wallet_trust.json")
    try:
        with open(path) as fh:
            return success(json.load(fh))
    except (FileNotFoundError, ValueError):
        return success({"whitelist": [], "blacklist": [], "wallets_with_data": 0})
